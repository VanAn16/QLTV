...
public class SachData {
    ...
    public static ResultSet showTextfield(String sql) {
     ...
     public static void InsertSach(Sach s) {
        String sql = "insert into SACH(Ma_Sach, Ten_Sach, Ten_Tac_gia, Nha_xb) values(?,?,?,?)";
        try {
            ps = Connect.getConnect().prepareStatement(sql);
    ...
    public boolean UpdateSach(Sach s) {
        try {
            ps = Connect.getConnect().prepareStatement("UPDATE SACH SET  Ten_Sach = ?, Ten_Tac_gia = ? ,Nha_xb = ? where Ma_Sach = ?");
