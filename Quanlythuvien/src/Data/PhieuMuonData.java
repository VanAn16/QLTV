...
public class PhieuMuonData {
    ...
    public static void InsertPhieu(PhieuMuon p) {
        String sql = "insert into PHIEU_MUON(Ma_Phieu_muon,id,Ma_Sach,Ngay_muon,Han_tra) values(?,?,?,?,?)";
        try {
            ps = Connect.getConnect().prepareStatement(sql);
            ps.setString(1, p.getMaMuon());
            ps.setString(2, p.getMaSinhVien());
    ...
    public boolean UpdatePhieu(PhieuMuon p) {
        try {
            ps = Connect.getConnect().prepareStatement("UPDATE PHIEU_MUON SET  id = ?, Ma_Sach = ?,"
                    + "Ngay_muon = ?, Han_tra = ? where Ma_Phieu_muon = ?");
            ps.setString(5, p.getMaMuon());
            ps.setString(1, p.getMaSinhVien());
